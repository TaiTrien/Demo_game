...
void LoadResources()
{
	for (int i = 0; i <= 8; i++) {
		if (i % 2 == 0 && i !=8) {
			gameObj[i] = new Mario(MARIO_TEXTURE_PATH);
			gameObj[i]->SetPosition(50.0f , 50.0f + i*20);
		}
		else if ( i%2 !=0 && i != 7) {
			gameObj[i] = new CGameObject(BRICK_TEXTURE_PATH);
			gameObj[i]->SetPosition(80.0f + i * 16, 150.0f );
		}
		else {
			gameObj[i] = new Ball(BALL_TEXTURE_PATH);
			gameObj[i]->SetPosition(10.0f +20*i , 10.0f + 20 *i);
		}
	}
